...
*/
const animalNames = [];
let sciAniName = zooAnimals.forEach((x) => 
...
const lowerCase = [];
let lowCaps = zooAnimals.map((x) => {
  return x.animal_name.toLowerCase()
})
lowerCase.push(lowCaps)
...
let lowCount = zooAnimals.filter(x => {
  return x.population < 5;
})
largerPopulation.push(lowCount)
